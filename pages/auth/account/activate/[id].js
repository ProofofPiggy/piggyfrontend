    ...
    return (
        <Layout>
            <div className="container" style={{minHeight:'40vh'}}>
                <h3 className="pb-4 pt-5">Hey {name}, ready to activate your account?</h3>
                {showLoading()}
                {error && error}
                {success && 
                (<div style={{display:'block'}}><p>You have successfully activated your account. Please signin.</p>
              <Link  href='/signinforadmin' ><button className="btn btn-outline-dark">    Sign In
              </button>
                    </Link>
                </div>)}
                {showButton && (
