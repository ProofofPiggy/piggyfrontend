...
if (options.nextCursor){
    ...
    const results =await fetch(`https://api.cloudinary.com/v1_1/${CLOUDINARY_CLOUD_NAME}/resources/search?${paramString}` , {
        headers:{
...
export async function getFolders(options = {}) {
    ...
    const results =await fetch(`https://api.cloudinary.com/v1_1/${CLOUDINARY_CLOUD_NAME}/folders` , {
        headers:{
        Authorization:`Basic ${Buffer.from(CLOUDINARY_API_KEY + ':' + CLOUDINARY_API_SECRET).toString('base64')}`
        }
    }).then(r=>r.json());
